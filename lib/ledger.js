...
export const createLedgerPayment = async (
  ...
  if (!hasRoleFields || isLegacyCombo) {
    ...
    if (paymentType) {
      // Base off 0 when there's no prior balance, so a party's first-ever transaction gets
      // the correct sign instead of defaulting to the raw (always-positive) totalAmount.
      if (legacyCompanyId) {
        companyTotal = (companyBalance.length ? companyBalance[0].amount : 0) - totalAmount;
      }
      if (legacyCustomerId) {
        customerTotal =
          paymentType === "REFUND"
            ? (customerBalance.length ? customerBalance[0].amount : 0) + totalAmount
            : (customerBalance.length ? customerBalance[0].amount : 0) - totalAmount;
      }
    }
