...
class ClaudeService {
  ...
  async callToolWithTimeout(toolName, toolInput, timeoutMs = 20000) {
    return Promise.race([
      this.mcpClient.callTool({ name: toolName, arguments: toolInput }),
    ...
    while (response.stop_reason !== "end_turn") {
      ...
      const toolResults = await Promise.all(
        toolUseBlocks.map(async (toolUse) => {
          let resultText;
          try {
            const result = await this.callToolWithTimeout(toolUse.name, toolUse.input);
