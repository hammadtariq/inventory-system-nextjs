...
class ClaudeService {
  ...
  //   onToken = null  → non-streaming, returns full reply in { reply, updatedMessages }
  //   onToken = fn    → streaming, calls fn(chunk) for each text token; reply is "" in return value
  async chat(messages, onProgress = null, onToken = null) {
    ...
    while (true) {
      ...
      // Stream every API call. Text tokens flow through onToken only on end_turn
      // rounds (tool_use rounds have no text blocks, so stream.on("text") never fires).
      ...
      if (onToken) {
        // chunk is the incremental text delta from this event
        stream.on("text", (chunk) => onToken(chunk));
      }
      ...
      if (response.stop_reason === "end_turn") {
        ...
        return {
          // When streaming, tokens were already delivered via onToken; reply is redundant
          // but kept for non-streaming callers (message.js) that need the full text.
